...
seen_ids = set()  # Houdt geziene advertenties bij in RAM (geen bestand nodig op Railway)
...
def search_marktplaats():
    """Zoek naar Liebherr vriezers via API + fallback."""
    ads = []
        ...
        if resp.status_code == 200:
            ...
            for listing in data.get('listings', []):
                try:
                    ...
                    continue
            print(f"API: {len(ads)} relevante ads gevonden")
        else:
    ...
    if not ads:
        try:
            ...
            if resp.status_code == 200:
                html = resp.text
                ids = re.findall(r'data-item-id="(\d+)"', html)
                ids += re.findall(r'/v/(\d+)/', html)
                ids = set(ids)
                ...
                print(f"Fallback: {len(ads)} IDs gevonden")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print("🚀 Bot draait, controleert elke 5 minuten...")
    
    # Oneindige loop met 5 minuten pauze
    while True:
        try:
            main_loop()
        except Exception as e:
            print(f"❌ Onverwachte fout: {e}")
            send_message(f"❌ *Fout in bot*\n`{str(e)[:200]}`")
        time.sleep(300)  # 5 minuten = 300 seconden
